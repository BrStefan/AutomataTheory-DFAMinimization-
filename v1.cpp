...
{
	char f[100]={0};
	for(int i=0;i<n;i++)f[v[i].stare1]++;
		for(int i=0;i<totale;i++)if(f[i]!=cate)return false;
            return true;
}
void algoritm(vector <int> stari,int nr_stari,vector<int> stari_finale,int nr_stari_finale,vector<tranzitie> functie, int nr_functii,vector <char> alfabet, int nr_alfabet)
{
    int matrice[nr_stari][nr_stari];
        ...
        /*for(int i=0;i<nr_stari;i++)
            {
                for(int j=0;j<nr_stari;j++)printf("%d ",matrice[i][j]);
                    printf("\n");
            }*/
    ...
    for(int i=0;i<nr_stari;i++)
        ...
        {
            e=0;
            for(int k=0;k<nr_stari_finale;k++)if(stari_finale[k]==i || stari_finale[k]==j)e++;
                if(e==1)matrice[i][j]=1;
        }
    ...
    do
    {
        ...
        for(int i=0;i<nr_stari;i++)
            for(int j=0;j<i;j++)
            {
                ...
                if(matrice[i][j]==0)
                {
                    //generam perechile (f(i,x),f(j,x)), cu x fiecare litera din alfabet
                    for(int k=0;k<nr_alfabet;k++)
                    {
                        ...
                        {
                            if(functie[x].alf==litera && functie[x].stare1==i)p1=functie[x].stare2;
                            if(functie[x].alf==litera && functie[x].stare1==j)p2=functie[x].stare2;
                        }
    ...
    for(int i=0;i<nr_stari;i++)
        for(int j=0;j<nr_stari;j++)matrice_stari[i][j]=-1;
    ...
    for(int i=0;i<nr_stari;i++)
        ...
        for(int i=0;i<nr_stari && matrice_stari[i][0]!=-1;i++)
            for(int j=0;j<nr_stari && matrice_stari[i][j]!=-1;j++)
                for(int k=0;k<nr_stari_finale;k++)if(matrice_stari[i][j]==stari_finale[k])finale[i]=1;

        printf("Starile rezultate sunt: ");
        for(int i=0;i<nr_stari && matrice_stari[i][0]!=-1;i++)
        {
            for(int j=0;j<nr_stari && matrice_stari[i][j]!=-1;j++)printf("%d",matrice_stari[i][j]);
            printf(" ");
        }

        printf("\n\nStarile initiale sunt: ");
        for(int i=0;i<nr_stari && matrice_stari[i][0]!=-1;i++)
        {
            for(int j=0;j<nr_stari && matrice_stari[i][j]!=-1;j++)if(finale[i]==0)printf("%d",matrice_stari[i][j]);
            printf(" ");
        }
        ...
        for(int i=0;i<nr_stari && matrice_stari[i][0]!=-1;i++)
        {
            for(int j=0;j<nr_stari && matrice_stari[i][j]!=-1;j++)if(finale[i]==1)printf("%d",matrice_stari[i][j]);
            printf(" ");
        }

        printf("\n\nVectorul de tranzitie este:\n");
        for(int i=0;i<nr_stari;i++)
            for(int k=0;k<nr_alfabet;k++)
                for(int j=0;j<nr_functii;j++)
                    if(matrice_stari[i][0]==functie[j].stare1 && alfabet[k]==functie[j].alf)
                    {
                        for(int x=0;matrice_stari[i][x]!=-1;x++)printf("%d",matrice_stari[i][x]);
                        printf("%c",alfabet[k]);
                        e=0;
                        ...
                        for(int j1=0;matrice_stari[i1-1][j1]!=-1;j1++)printf("%d",matrice_stari[i1-1][j1]);
                        printf("\n");
                        break;
                    }
       ...
       /* printf("\n\n\n");
        for(int i=0;i<nr_stari;i++)
        {
            for(int j=0;j<nr_stari;j++)printf("%d ",matrice_stari[i][j]);
                printf("\n");
        }*/
...
int main()
{
                        ...
                        alfabet,nr_alfabet)==true)
    {
    	if(verifica_complet(functii,nr_functii,nr_alfabet,nr_stari_initiale,stari)==true)
    	{
            ...
            algoritm(stari,nr_stari_initiale,
                     stari_finale,nr_stari_finale,
                     functii,nr_functii,
                     alfabet,nr_alfabet);
            
    	}
        else printf("Incomplet");
    }
    return 0;
}